...
main <- function(){
  ...
  plot1 <- ggplot(data, aes(x = factor(x = quality), alcohol)) +
  ...
  plot2 <- ggplot(data2, aes(x = value)) +
  ...
  # print(head(test))
  ggsave(paste0(output_dir, '/alcohol.png'), plot1)
  ggsave(paste0(output_dir, '/all_vars.png'), plot2)
